...
import { chunks, type Chunk } from "@/data/chunks";
...
function buildQuestions(): Question[] {
  const selected = shuffle(chunks).slice(0, QUESTIONS_PER_SESSION);
  return selected.map((chunk) => ({
    chunk,
    options: shuffle([chunk.japanese, ...chunk.wrongOptions]),
    correct: chunk.japanese,
  }));
}
...
export default function ChunkPage() {
  ...
  const speak = useCallback(() => {
    if (current) speakEnglish(current.chunk.english);
  }, [current]);

  useEffect(() => {
    if (phase === "question") setShowDetail(false);
  }, [index, phase]);
  ...
  function handleAnswer(option: string) {
    ...
    recordAnswer(current.chunk.id, correct);
    if (correct) {
      addXP(XP_CORRECT);
    ...
    const stop = startListening(
      ...
      ({ transcript }) => {
        ...
        const match = current?.options.find(
          (o) => o.replace(/[　 ]/g, "").includes(transcript.replace(/[　 ]/g, "").slice(0, 4))
        );
  ...
  if (phase === "complete") {
    return (
      <div className="flex flex-col items-center justify-center flex-1 px-6 gap-6 text-center animate-celebration">
        ...
        <button
          onClick={() => router.push("/")}
          className="w-full bg-[#58CC02] text-white font-bold text-lg rounded-2xl py-4 active:scale-95 transition-transform"
        >
          ホームへ戻る
        </button>
      </div>
    ...
    <div className="flex flex-col flex-1">
      ...
      <div className="flex items-center gap-3 px-4 pt-4 pb-2">
        ...
        <div className="flex-1 bg-gray-200 rounded-full h-4">
          <div
            className="bg-[#58CC02] h-4 rounded-full transition-all duration-500"
      ...
      {/* Question area */}
      <div className={`flex flex-col flex-1 px-4 pt-4 gap-4 ${phase === "feedback" ? "pb-64" : "pb-4"}`}>
        ...
        <div className="bg-white rounded-2xl shadow-md p-5">
          <div className="flex items-start justify-between gap-3">
            <p className="text-xl font-bold text-gray-800 leading-relaxed flex-1">
              {current.chunk.english}
            </p>
            <button
              onClick={speak}
              className="bg-[#1CB0F6] text-white rounded-xl p-3 text-xl active:opacity-60 shrink-0"
            >
              🔊
            </button>
          </div>
          <p className="text-sm text-gray-400 mt-2">{current.chunk.hint}</p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {current.options.map((option) => {
            ...
            if (selected) {
              if (option === current.correct) style = "bg-[#D7FFB8] border-2 border-[#58CC02] text-gray-800";
            ...
            return (
              <button
                ...
                disabled={!!selected}
                className={`${style} rounded-2xl px-4 py-4 text-left font-medium text-base transition-colors ${
                  selected && option === selected && option !== current.correct ? "animate-shake" : ""
                }`}
              >
                {option}
              </button>
          ...
          <div className={`w-full max-w-md px-4 pb-8 pt-5 ${isCorrect ? "bg-[#D7FFB8]" : "bg-[#FFD5D5]"}`}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className={`text-xl font-bold ${isCorrect ? "text-[#46a302]" : "text-[#FF4B4B]"}`}>
                {isCorrect ? "すごい！正解！" : "まちがい"}
              </p>
            </div>
            {!isCorrect && (
              <p className="text-sm text-gray-700 mb-2">
                正解：<span className="font-bold">{current.correct}</span>
              </p>
            )}
            <button
            ...
            >
              {showDetail ? "解説を閉じる ▲" : "詳しい解説を見る ▼"}
            </button>
            {showDetail && (
              <div className="bg-white/60 rounded-xl p-3 mb-3 text-sm">
                <table className="w-full mb-2">
                  <thead>
                    <tr className="text-xs text-gray-500 border-b border-gray-200">
                      <th className="text-left py-1">英語</th>
                      <th className="text-left py-1">品詞</th>
                      <th className="text-left py-1">意味</th>
                    </tr>
                  </thead>
                  <tbody>
                    {current.chunk.words.map((w, i) => (
                      <tr key={i} className="border-b border-gray-100">
                        <td className="py-1 font-semibold">{w.word}</td>
                        <td className="py-1 text-gray-500">{w.role}</td>
                        <td className="py-1">{w.meaning}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <p className="text-xs text-gray-600 mb-1"><span className="font-bold">語順：</span>{current.chunk.wordOrder}</p>
                <p className="text-xs text-gray-600"><span className="font-bold">使い方：</span>{current.chunk.usageTip}</p>
              </div>
            )}
            <button
              onClick={handleNext}
